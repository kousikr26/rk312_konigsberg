...
end_date="20-6-2020"
end_time="21:30:55"
towers_df=pd.read_csv('bhopal_towers.csv')
samples=towers_df.sample(n=100)
towerIDS=[]
for index,row in samples.iterrows():
    # print(str(row['mcc'])+str(row['net'])+'-'+str(row['area'])+'-'+str(row['cell']))
    x=str(row['mcc'])+str(row['net'])+'-'+str(row['area'])+'-'+str(row['cell'])
    towerIDS.append(x)
